...
int main() {
    ...
    printf("[DEBUG] ui_show_menu succeeded, calling game_init...\n");
    ...
    printf("[DEBUG] game_init done, player_count=%d\n", gs.player_count);
    
    /* 发初始手牌并显示 */
    game_deal_initial_cards(&gs);
    printf("[DEBUG] Initial cards dealt\n");
    ui_show_initial_hand(&gs);
    printf("[DEBUG] Initial hand shown, entering game loop...\n");
